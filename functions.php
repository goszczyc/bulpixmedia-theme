...
require_once(__DIR__ . '/inc/ajax/ajax.php');
...
{
  if(get_post_type() == 'offer') return 43;
  return 9;
}
...
function pagination_bar($custom_query, $numbers_class)
{
  ...
  if ($total_pages > 1) {
    ...
    echo get_previous_posts_link(translate('Previous', 'bulpix'));
    ...
    echo '</div>';
    echo get_next_posts_link(translate('Next', 'bulpix'), $custom_query->max_num_pages);
  }
}

function my_theme_posts_next_link_attributes()
{
  return 'class="btn btn--secondary pagination-nav__next"';
}
function my_theme_posts_prev_link_attributes()
{
  return 'class="btn btn--secondary pagination-nav__prev"';
}
...
function language_selector()
{
  ...
  if (!empty($languages)) {
    foreach ($languages as $l) {
      ...
      $class = 'haeder__lang-menu-element';

      if (!$l['active']) {
        echo '<span class="haeder__lang-menu-separator"> | </span><a class="' . $class . '" href="' . $l['url'] . '">' . $code . '</a>';
      }
    }
  }
}
...
function language_current()
{
  ...
  if (!empty($languages)) {
    foreach ($languages as $l) {
      $code = $l['language_code'];
      $class = 'haeder__lang-menu-element haeder__lang-menu-element--current';
      if ($l['active']) {
